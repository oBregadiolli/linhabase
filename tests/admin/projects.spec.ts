...
test.describe('Admin — Gestão de Projetos', () => {
  test('lista projetos existentes', async ({ page }) => {
    ...
    const hasAlpha = await page.getByText('E2E Projeto Alpha').isVisible({ timeout: 10_000 }).catch(() => false)
    const hasBeta = await page.getByText('E2E Projeto Beta').isVisible({ timeout: 5_000 }).catch(() => false)
